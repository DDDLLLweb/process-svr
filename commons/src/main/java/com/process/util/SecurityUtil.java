...
public abstract class SecurityUtil {
    ...
    private static final String XSRF_NAME = "ps_csrf";
    private static final String XSRF_PARAM_NAME = "_csrf";
     ...
     * @param response
     */
    public static void createCsrfToken(HttpServletRequest request, HttpServletResponse response) {
        String token = UUID.randomUUID().toString();
        Cookie tokenCookie = new Cookie(XSRF_PARAM_NAME, token);
        tokenCookie.setHttpOnly(true);
        tokenCookie.setPath(request.getContextPath() + "/");
        response.addCookie(tokenCookie);
        response.setHeader("ps-result-code", token);
        log.trace("[CORE-CSRF] token created: {}@{}", tokenCookie.getValue(), tokenCookie.getPath());
    }
